...
void main() {
  group('getPathFromTreeUri', () {
    test('should return the root folder path from the content uri', () {
      expect(
        ContentUriHelper.getPathFromTreeUri(
  ...
  group('getEntityNameFromPath', () {
    ...
    test('should return the entity name from the path with a folder in SD card',
        () {
      expect(
        ContentUriHelper.getEntityNameFromPath(
            '1234-5678:Documents/Office Lens'),
        'Office Lens',
      );
    });
  });
  ...
  group('guessRelativePathFromPickedFileContentUri', () {
    ...
    test(
        'should return the relative path from the picked file content uri without the folder',
        () {
      expect(
        ContentUriHelper.guessRelativePathFromPickedFileContentUri(
          folderContentUri:
    ...
    test(
        'should return the relative path from the picked file content uri in SD card',
        () {
      expect(
        ContentUriHelper.guessRelativePathFromPickedFileContentUri(
          folderContentUri:
    ...
    test(
        'should return the relative path from the picked file content uri in SD card root',
        () {
      expect(
        ContentUriHelper.guessRelativePathFromPickedFileContentUri(
          folderContentUri:
  ...
  group('encodeTreeUri', () {
    ...
    test('should return the encoded tree uri with a folder in SD card', () {
      expect(
        ContentUriHelper.encodeTreeUri(
            'content://com.android.externalstorage.documents/tree/1234-5678:LocalSend/subFolder'),
