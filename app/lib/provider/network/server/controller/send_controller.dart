...
class SendController {
  ...
  void installRoutes({required SimpleServerRouteBuilder router, required String alias, required String fingerprint}) {
        ...
        if (session != null && session.responseHandler == null && session.ip == request.ip) {
          ...
          return await request.respondJson(
            ...
            body: ReceiveRequestResponseDto(
              ...
              files: {for (final entry in state.webSendState!.files.entries) entry.key: entry.value.file},
            ).toJson(),
          );
        }
      }

      final pinCorrect = await checkPin(server: server, pin: state.webSendState!.pin, pinAttempts: state.webSendState!.pinAttempts, request: request);
      if (!pinCorrect) {
        return;
      }
        ...
        (oldState) => oldState!.copyWith(
          webSendState: oldState.webSendState!.copyWith(
            sessions: {
              ...
              sessionId: WebSendSession(sessionId: sessionId, responseHandler: streamController, ip: request.ip, deviceInfo: request.deviceInfo),
            },
          ),
        ),
      );
...
extension on WebSendState {
  WebSendState updateSession({required String sessionId, required WebSendSession Function(WebSendSession oldSession) update}) {
    return copyWith(sessions: {...sessions}..update(sessionId, (session) => update(session)));
  }
}
