...
class _StringsSendTabEn {
  ...
  String get help =>
      'Please ensure that the desired target is also in the same wifi network.';
...
class _StringsTroubleshootPageEn {
  ...
  String get subTitle =>
      'This app does not work as expected? Here you can find common solutions.';
...
class _StringsWebSharePageEn {
  ...
  String get encryptionHint =>
      'LocalSend uses a self-signed certificate. You need to accept it in the browser.';
...
class _StringsTroubleshootPageNoConnectionEn {
  ...
  String get solution =>
      'The problem exists on both sides? Then you need to make sure that both devices are in the same wifi network and share the same configuration (port, multicast address, encryption). The wifi may not allow communication between participants. In this case, this option must be enabled on the router.';
}
...
class _StringsDialogsQuickSaveNoticeEn {
  ...
  String get content =>
      'File requests are automatically accepted. Be aware that everyone in the local network can send you files.';
}
