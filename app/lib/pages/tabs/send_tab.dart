              ...
              children: [
                ...
                ...vm.nearbyDevices.map((device) {
                  ...
                  return Padding(
                    ...
                    child: Hero(
                      tag:
                          'device-${device.fingerprint}-${device.signalingId ?? device.ip ?? device.alias}',
...
class _MultiSendDeviceListTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ref = context.ref;
    final session = ref.watch(sendProvider).values.firstWhereOrNull((s) {
      if (s.target.fingerprint == device.fingerprint) {
        return true;
      }
      if (s.target.signalingId != null &&
          s.target.signalingId == device.signalingId) {
        return true;
      }
      return s.target.ip != null && s.target.ip == device.ip;
    });
