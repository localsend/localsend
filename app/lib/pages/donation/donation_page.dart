...
class DonationPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder(
      ...
      // [FOSS_REMOVE_START]
      init: (context) async => context.redux(purchaseProvider).dispatchAsync(
          FetchPricesAndPurchasesAction()), // ignore: discarded_futures
      // [FOSS_REMOVE_END]
      builder: (context, vm) {
        return Scaffold(
          appBar: AppBar(
        ...
        ...PurchaseItem.values.map((item) {
          return Padding(
            ...
            child: FilledButton.icon(
              onPressed:
                  vm.purchased.contains(item) ? null : () => vm.purchase(item),
  ...
  Widget build(BuildContext context) {
    return Wrap(
      ...
      children: [
        TextButton.icon(
          onPressed: () async {
            await launchUrl(Uri.parse('https://github.com/sponsors/Tienisto'),
        ...
        TextButton.icon(
          onPressed: () async {
            await launchUrl(Uri.parse('https://ko-fi.com/tienisto'),
                mode: LaunchMode.externalApplication);
          },
          icon: const Icon(Icons.open_in_new),
