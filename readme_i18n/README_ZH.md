...
> [!CAUTION]
> **非官方 MSIX 预览版：** 你可以在 [localsend.ob-buff.dev](https://localsend.ob-buff.dev/) 获取并尝试最新提交的构建版本。该版本不保证稳定性，同时所有自定义代码调整均在此网站上列出。
...
|Linux    | 不适用     | 依赖：Gnome 需`xdg-desktop-portal` 和 `xdg-desktop-portal-gtk`；KDE 需 `xdg-desktop-portal` 和 `xdg-desktop-portal-kde`    |
...
LocalSend 使用安全通信协议，允许设备之间通过 REST API 进行通信。所有数据都通过 HTTPS 安全地发送，并且 TLS/SSL 证书会在每台设备上动态生成，确保最大的安全性。
